import { fetchRequestedTo } from "@/app/lib/channel/data";
...
export default async function Requests() {
  ...
  const channels = await fetchRequestedTo(user.id);
          ...
          pendingChannels.map((channel: any) => {
            return (
              <Suspense key={channel.id} fallback={<Loading />}>
                <Contact currentUser={user.id} userId={channel.requestedBy} />
              </Suspense>
            );
          })
        ) : (
          <p className="text-neutral-500">There are no unconfirmed requests</p>
        )}
      </div>
    </>
  );
    ...
    <>
      <div className="p-3 bg-white shadow-md rounded-lg h-18">
        <div className="flex flex-col items-start h-[100%]">
          ...
          <Buttons currentUser={currentUser} userId={userId} />
        </div>
      </div>
    </>
  );
